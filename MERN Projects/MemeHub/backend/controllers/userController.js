...
const createToken = (email) => {
  return jwt.sign({ email }, process.env.Jwt_sec, {
    expiresIn: "5d",
  });
};
...
export const googleLogin = async (req, res) => {
  try {
    const { name, email } = req.body; // Data from Firebase on the client
    ...
    // Note: No password is saved, as per your User.js model
...
export const myProfile = async (req, res) => {
  try {
    // req.user is just the email, thanks to your isAuth.js middleware
...
// 5. DELETED: loginUser (OTP) and verifyUser (OTP)
// Those functions are no longer needed.