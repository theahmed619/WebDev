...
const Videos = () => {
  ...
  const fetchVideos = async () => {
    try {
      ...
      setBlogs(data.blogs.filter(blog => blog.blogVideo));
    } catch (error) {
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-gray-500 font-semibold mt-1">
                  {blogs.length} stories in motion
                </p>
              </div>
            </div>
            <div className="h-1.5 bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 rounded-full w-32"></div>
          </div>
            ...
            <div className="flex flex-col items-center justify-center py-20">
              <div className="bg-white rounded-3xl shadow-xl p-12 text-center border border-gray-100">
                <div className="bg-gradient-to-br from-blue-100 to-indigo-100 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6">
                  <PlayCircle className="w-12 h-12 text-indigo-600" strokeWidth={2} />
                </div>
                <h3 className="text-2xl font-bold text-gray-800 mb-2">No Videos Yet</h3>
                <p className="text-gray-500 font-medium">Start by uploading your first video!</p>
              </div>
            </div>
          ) : (
